...
const CommentItem = ({ item }: TProps) => {
  ...
  const { isLoading, isSuccessDelete, isSuccessEdit } = useSelector(
  ...
  const handleReply = (
    comment: string,
    isEdit: boolean,
    itemComment?: TCommentItemProduct,
  ) => {
    if (comment) {
      if (user) {
        if (isEdit) {
              ...
              <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                  ...
                  <Typography color="secondary">
                    {getTimePast(new Date(item.createdAt), t)}
                  </Typography>
                </Box>
              </Box>
              {editComment ? (
