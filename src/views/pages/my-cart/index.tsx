...
// ** Utils
import { cloneDeep, createUrlQuery, formatNumberToLocal } from "src/utils";
...
const MyCartPage: NextPage<TProps> = () => {
  ...
  const memoItemsSelectedProduct = useMemo(() => {
    ...
    selectedRows.forEach((idSelected) => {
      const findItem: any = orderItems.find(
        (item: TItemOrderProduct) => item.product === idSelected,
      );
      if (findItem) {
  ...
  const memoTotalSelectedProduct = useMemo(() => {
    const total = memoItemsSelectedProduct?.reduce(
      (result, current: TItemOrderProduct) => {
        const currentPrice =
          current?.discount > 0
  ...
  useEffect(() => {
    const productSelected = searchParams.get("selected") as string;
    if (productSelected) {
  ...
  const handleChangeCheckAll = () => {
    const isCheckedAll = memoListAllProductIds.every((id) =>
  ...
  const handleDeleteMany = () => {
    ...
    const filteredItems = cloneOrderItems.filter(
      (item: TItemOrderProduct) => !selectedRows.includes(item.product),
    );
    if (user) {
      dispatch(
        updateProductToCart({
  ...
  const handleNavigateCheckoutProduct = () => {
    const formatData = memoItemsSelectedProduct.map((item) => ({
    ...
    <>
      ...
      >
              ...
              <Box sx={{ width: "calc(10% - 100px)" }}>
                <Tooltip title={t("Select_all")}>
                  <Checkbox
                    ...
                    checked={memoListAllProductIds.every((id) =>
                      selectedRows.includes(id),
                    )}
                  />
                </Tooltip>
        ...
        <Box sx={{ display: "flex", justifyContent: "flex-end", gap: "2px" }}>
          ...
          >
            {formatNumberToLocal(memoTotalSelectedProduct, {
