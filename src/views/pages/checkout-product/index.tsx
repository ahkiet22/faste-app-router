...
const CheckoutProductPage: NextPage<TProps> = () => {
  ...
  >([]);
  const [optionDeliveries, setOptionDeliveries] = useState<
  ...
  const memoQueryProduct = useMemo(() => {
    ...
    const data: any = searchParams.get("state");
    if (data) {
      const states: TCheckoutState = decrypt(data) as TCheckoutState;
      result.totalPrice = states.totalPrice || 0;
      result.productsSelected = states.productsSelected
        ? handleFormatDataProduct(states.productsSelected)
        : [];
    }
  ...
  useEffect(() => {
    const dataQuery = searchParams.get("state");
    if (dataQuery) {
  ...
  const memoNameCity = useMemo(() => {
    const findCity = optionCities.find(
      (item) => item.value === memoAddressDefault?.city,
    );
  ...
  const memoPriceShipping = useMemo(() => {
    const findItemPrice = optionDeliveries?.find(
      (item) => item.value === deliverySelected,
    );
  ...
  const handlePaymentTypeOrder = (
    type: string,
    data: { orderId: string; totalPrice: number },
  ) => {
    switch (type) {
  ...
  const handleOrderProduct = () => {
    ...
    dispatch(
      ...
      const findPayment = optionPayments.find(
        (item) => item.value === idPaymentMethod,
      );
      if (findPayment) {
  ...
  const handleGetListPaymentMethod = async () => {
    ...
    await getAllPaymentTypes({ params: { limit: -1, page: -1 } })
      .then((res) => {
        if (res.data) {
          setOptionPayments(
            res?.data?.paymentTypes?.map(
              (item: { name: string; _id: string; type: string }) => ({
                label: item.name,
                value: item._id,
                type: item.type,
              }),
  ...
  const handleGetListDeliveryMethod = async () => {
    ...
    await getAllDeliveryTypes({ params: { limit: -1, page: -1 } })
      .then((res) => {
        if (res.data) {
          setOptionDeliveries(
            res?.data?.deliveryTypes?.map(
              (item: { name: string; _id: string; price: string }) => ({
                label: item.name,
                value: item._id,
                price: item.price,
              }),
  ...
  const handleChangeAmountCart = (items: TItemOrderProduct[]) => {
    ...
    const filterListOrder = listOrderItems.filter(
      (item: TItemOrderProduct) => item.amount,
    );
    if (user) {
      dispatch(
        updateProductToCart({
      ...
      >
        {memoQueryProduct?.productsSelected?.length > 0 ? (
          <Fragment>
            ...
            >
              <Typography
                sx={{ width: "80px", marginLeft: "20px", fontWeight: 600 }}
              >
                {t("Image")}
              </Typography>
              <Typography sx={{ flexBasis: "35%", fontWeight: 600 }}>
                {t("Name_product")}
              </Typography>
              ...
              <Typography sx={{ flexBasis: "10%", fontWeight: 600 }}>
                {t("Count")}
              </Typography>
            </Box>
            <Divider />
            ...
            >
              {memoQueryProduct?.productsSelected?.map(
                (item: TItemOrderProduct, index: number) => {
                  return (
                    <Fragment key={item.product}>
                        ...
                        <Box sx={{ flexBasis: "20%" }}>
                          <Typography
                            ...
                            sx={{
                              ...
                              textDecoration:
                                item.discount > 0 ? "line-through" : "normal",
                              fontSize: "18px",
                            }}
                          >
                      ...
                      {index !==
                        memoQueryProduct?.productsSelected?.length - 1 && (
                        <Divider />
                      )}
                    </Fragment>
                  );
      ...
      >
              ...
              {optionDeliveries.map((delivery) => {
                return (
                  <FormControlLabel
                    ...
                    control={
                      <Radio checked={deliverySelected === delivery.value} />
                    }
              ...
              {optionPayments.map((payment) => {
                return (
                  <FormControlLabel
                    ...
                    value={payment.value}
                    control={
                      <Radio checked={paymentSelected === payment.value} />
                    }
                    label={payment.label}
                  />
                );
        ...
        >
          <Box sx={{ display: "flex", gap: "2px" }}>
            ...
            <Typography sx={{ fontSize: "20px", width: "200px" }}>
              {formatNumberToLocal(memoQueryProduct.totalPrice, {
                language: (i18n.language as "vi") || "en",
              })}
            </Typography>
          </Box>
          <Box sx={{ display: "flex", gap: "2px" }}>
            ...
            <Typography sx={{ fontSize: "20px", width: "200px" }}>
              {formatNumberToLocal(memoPriceShipping, {
                language: (i18n.language as "vi") || "en",
              })}
            </Typography>
          </Box>
          <Box sx={{ display: "flex", gap: "2px" }}>
            <Typography
              sx={{ fontSize: "20px", width: "200px", fontWeight: 600 }}
            >
              {t("Sum_money")}:
            </Typography>
            <Typography
            ...
            >
              {formatNumberToLocal(
                +memoQueryProduct.totalPrice + +memoPriceShipping,
                {
                  language: (i18n.language as "vi") || "en",
                },
              )}
