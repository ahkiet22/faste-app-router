...
export const getTextFromHTML = (html: string): string => {
  if (!html) return ''
  // Loại bỏ tất cả thẻ HTML, giữ lại khoảng trắng
  return html
    .replace(/<script[\s\S]*?>[\s\S]*?<\/script>/gi, ' ') // loại bỏ <script>…</script>
    .replace(/<style[\s\S]*?>[\s\S]*?<\/style>/gi, ' ') // loại bỏ <style>…</style>
    .replace(/<\/?[^>]+(>|$)/g, ' ') // loại bỏ các tag còn lại
